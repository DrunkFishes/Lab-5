...
/* This example accompanies the book
   "Embedded Systems: Real Time Interfacing to Arm CortexM Microcontrollers",
...
unsigned int steps,turn, run; //counters
unsigned int goForward; 
unsigned int turningLeft;
unsigned int keepGoing;
unsigned int stop;		// flag for when object sensed
unsigned int end;  // flag that stops the first loop
unsigned int tog, check; 
unsigned int count; // counter to stop interrupt flux


// flags for 2nd button
unsigned int times,mult, walk; //counters
...
int main(void){
  ...
  while(1){
		...
		if(count == 100){ // delay to stop random changes
			tog = 0;
			count = 0; 
			keepGoing = 0; 
			check= 1;
			...
			if (stop){ // if the sensor reads an object
					LIGHT = red 
				
			}
			if (~stop){  // object is now gone
				end = 1; 
				}
			}
...
void GPIOPortB_Handler(void){
	...
	if (keepGoing == 1 || check== 1){ // this will prevent  the code from
		tog = 1; 										 // interrupting before needed
	}
}
